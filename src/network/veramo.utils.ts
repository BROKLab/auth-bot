...
import { DataStore, DataStoreORM, DIDStore, Entities, IDataStoreORM, KeyStore } from '@veramo/data-store';
...
export function getAgentConfig(dbConnection: Promise<Connection>, defaultDidProvider: string, secretKey?: string) {
  return {
    plugins: [
      new KeyManager({
        store: secretKey ? new KeyStore(dbConnection, new SecretBox('29739248cad1bd1a0fc4d9b75cd4d2990de535baf5caadfdf8d8f86664aa830c')) : new KeyStore(dbConnection),
        kms: {
...
export function initAgent(
  ...
  return createAgent<IDIDManager & IKeyManager & IDataStore & IDataStoreORM & IResolver & ICredentialIssuer>(agentConfig);
}
